...
UReplicationGraphV::UReplicationGraphV()
{
...
void UReplicationGraphV::InitGlobalActorClassSettings()
{
	...
	for (auto ClassMapIt = ClassRepNodePolicies.CreateIterator(); ClassMapIt; ++ClassMapIt)
	{
		...
		const EClassRepNodeMapping* ParentMapping = ClassRepNodePolicies.Get((UClass*)ParentNativeClass);
...
#if WITH_GAMEPLAY_DEBUGGER
	AGameplayDebuggerCategoryReplicator::NotifyDebuggerOwnerChange.AddUObject(this, &UReplicationGraphV::OnGameplayDebuggerOwnerChange);
#endif
}

void UReplicationGraphV::InitGlobalGraphNodes()
{
...
void UReplicationGraphV::InitConnectionGraphNodes(UNetReplicationGraphConnection* RepGraphConnection)
{
...
void UReplicationGraphV::OnRemoveConnectionGraphNodes(UNetReplicationGraphConnection* RepGraphConnection)
{
	ULocusReplicationConnectionGraph* LocusConnManager = Cast<ULocusReplicationConnectionGraph>(RepGraphConnection);
	if (LocusConnManager)
	{
...
void UReplicationGraphV::RemoveClientConnection(UNetConnection* NetConnection)
{
...
void UReplicationGraphV::RouteAddNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalInfo)
{
...
void UReplicationGraphV::RouteRemoveNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo)
{
	EClassRepNodeMapping Policy = GetMappingPolicy(ActorInfo.Class);
...
void UReplicationGraphV::ResetGameWorldState()
{
	...
	auto EmptyConnectionNode = [](TArray<UNetReplicationGraphConnection*>& Connections1)
	{
		for (UNetReplicationGraphConnection* ConnManager : Connections1)
		{
...
void UReplicationGraphV::AddDependentActor(AActor* ReplicatorActor, AActor* DependentActor)
{
...
void UReplicationGraphV::RemoveDependentActor(AActor* ReplicatorActor, AActor* DependentActor)
{
	if (ReplicatorActor && DependentActor)
	{
		CHECK_WORLDS(ReplicatorActor);
...
void UReplicationGraphV::ChangeOwnerOfAnActor(AActor* ActorToChange, AActor* NewOwner)
{
...
void UReplicationGraphV::SetTeamForPlayerController(APlayerController* PlayerController, FName NextTeam)
{
	if (PlayerController)
	{
...
void UReplicationGraphV::RouteAddNetworkActorToConnectionNodes(EClassRepNodeMapping Policy, const FNewReplicatedActorInfo& ActorInfo, FGlobalActorReplicationInfo& GlobalInfo)
{
...
void UReplicationGraphV::RouteRemoveNetworkActorToConnectionNodes(EClassRepNodeMapping Policy, const FNewReplicatedActorInfo& ActorInfo)
{
	if (ULocusReplicationConnectionGraph* ConnManager = FindLocusConnectionGraph(ActorInfo.GetActor()))
	{
		switch (Policy)
...
void UReplicationGraphV::HandlePendingActorsAndTeamRequests()
{
...
class ULocusReplicationConnectionGraph* UReplicationGraphV::FindLocusConnectionGraph(const AActor* Actor)
{
	if (Actor)
	{
...
#if WITH_GAMEPLAY_DEBUGGER
void UReplicationGraphV::OnGameplayDebuggerOwnerChange(AGameplayDebuggerCategoryReplicator* Debugger, APlayerController* OldOwner)
{
...
void UReplicationGraphV::PrintRepNodePolicies()
{
	UEnum* Enum = FindObject<UEnum>(ANY_PACKAGE, TEXT("EClassRepNodeMapping"));
	if (!Enum)
	{
...
EClassRepNodeMapping UReplicationGraphV::GetMappingPolicy(const UClass* Class)
{
	EClassRepNodeMapping* PolicyPtr = ClassRepNodePolicies.Get((UClass*)Class);
...
{
	UReplicationGraphV* ReplicationGraph = Cast<UReplicationGraphV>(GetOuter());
...
FAutoConsoleCommandWithWorldAndArgs ShooterPrintRepNodePoliciesCmd(TEXT("LocusRepGraph.PrintRouting"), TEXT("Prints how actor classes are routed to RepGraph nodes"),
	FConsoleCommandWithWorldAndArgsDelegate::CreateLambda([](const TArray<FString>& Args, UWorld* World)
		{
			for (TObjectIterator<UReplicationGraphV> It; It; ++It)
			{
